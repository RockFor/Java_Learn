 ...
 */
/*
* Задание: создать класс Булева матрица(n*m). Поля: размерность и значение матрицы.
* Реализовать методы логического сложения и отрицания матрицы. В классе определить конструкторы
 * и методы доступа к полям, поля должны быть Private.
Нашла в Интернете пример создания булевых матриц, но программа не работает. Подскажите, пожалуйста,
 что надо изменить в коде и как реализовать методы сложения и отрицания матриц?
*
*
*
* */
...
class Matrix {
    ...
    void run() {
        ...
        inverseMatrix(firstMatrix);
        odinMatrix(firstMatrix, "Количество цифр");
    }
